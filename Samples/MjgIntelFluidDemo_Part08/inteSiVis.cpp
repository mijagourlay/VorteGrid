...
static const Vec3 sGravityAcceleration( 0.0f , 0.0f , -10.0f ) ; ///< Acceleration due to gravity
...
static inline Vec4 GetColorFromRamp( const Vec4 * colorRamp , int numColorsInRamp , float zeroToOne )
{
    ...
    const float         spanBetweenRampElements = 1.0f / float( numColorsInRamp - 1 ) ;
    ...
    , mDecorations( DECO_NONE )
{
    ...
    // Set up particle system
    {
        ...
        // Finding bounding box must occur after emission
        // (because bounding box must include all particles, including those just emitted)
        // and before VortonSim
        // (because VortonSim uses the bounding box to create a grid of appropriate size
        // to capture all relevant terms of the vorticity equation).
        mPclOpFindBoundingBox.mParticles = & mTracers ;
        mParticleProcess.mParticleOps.PushBack( & mPclOpFindBoundingBox ) ;

        // PclOpVortonSim must occur after finding bounding box and before advection.
        // Advection will move particles outside the bounding box, at which point
        // the grids that hold the vorticity equation terms will become invalid.
        ...
        // because advection uses the total velocity to move particles,
        // and Wind adds to that velocity.
...
void InteSiVis::InitialConditions( unsigned ic )
{
    ...
    {   // Switch on initial conditions
        ...
        break ;
        case 2: // Projectile
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 1024 , VortexNoise( Vec3( 4.0f * fThickness , 0.5f * fThickness , 0.5f * fThickness ) ) ) ;
            // Add a sphere
        ...
        case 3: // Projectile with spin about longitudinal axis
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 2048 , VortexNoise( Vec3( 4.0f * fThickness , 0.5f * fThickness , 0.5f * fThickness ) ) ) ;
            // Add a sphere
        ...
        case 5: // Spinning sphere
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 2048 , VortexNoise( Vec3( fThickness , fThickness , fThickness ) ) ) ;
            // Add a sphere
            GetSpheres().PushBack( RbSphere( Vec3( 0.f , 0.0f , 0.0f ) , Vec3( 0.f , 0.0f , 0.0f ) , 1000.0f , 0.2f ) ) ;
        ...
        case 6: // Sphere in spinning fluid
            AssignVortons( vortons , 2.0f * fMagnitude , numVortonsMax , VortexTube( fThickness , /* variation */ 0.0f , /* width */ 2.0f * fThickness , 2 , 0 ) ) ;
            // Add a sphere
        ...
        {
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 256 , VortexNoise( Vec3( 0.5f * fThickness , 0.5f * fThickness , 0.5f * fThickness ) ) ) ;
            // Add a sphere
        ...
        case 12: // ball of initially stationary heavy fluid
            AssignVortons( vortons , 1.0f , numVortonsMax , HeavyBall( fRadius , 10.0f ) , Vec3( 0.0f , 0.0f , 5.0f ) ) ;
            mCamera.SetTarget( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
            mCamera.SetEye( Vec3( 0.0f , -10.0f , 0.0f ) ) ;
            numTracersPerCellCubeRoot = 6 ;
            vortonsImplyingTracers = ( const Vector< Particle > * )( & vortons ) ;
        ...
        case 13: // Two balls of initially stationary fluid, one heavy, one light, heavy one above light one.
            AssignVortons( vortons , 1.0f , numVortonsMax , HeavyBall( fRadius , -1.0f ) , Vec3( 0.0f , 0.0f , -2.0f ) ) ;
            AssignVortons( vortons , 1.0f , numVortonsMax , HeavyBall( fRadius ,  1.0f ) , Vec3( 0.0f , 0.0f ,  2.0f ) ) ;
            mCamera.SetTarget( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
    ...
    {   // For some scenarios, place a point light inside the cloud to make it seem to glow from within.
        case 0: // vortex ring -- vorticity in [0,1]
        case 1: // "jet" vortex ring -- velocity in [0,1]
            SetGlow( Particles::ComputeCenterOfMass( mTracers ) ) ;
        break ;
        default:
    ...
    const float massPerParticle = Particles::ComputeMassPerParticle( vortonSim.GetGrid() , numTracersPerCell , vortonSim.GetAmbientDensity() ) ;
    Particles::Emit( mTracers , vortonSim.GetGrid() , numTracersPerCellCubeRoot , massPerParticle , vortonsImplyingTracers ) ;
...
static void DrawGrid( const UniformGrid<Vec3> & grid , const QdMaterial & material , bool bDrawCells )
{
    ...
    {
        // Draw each grid cell.
        const int nx    = grid.GetNumCells( 0 ) ;
        const int ny    = grid.GetNumCells( 1 ) ;
        const int nz    = grid.GetNumCells( 2 ) ;
        const int npx   = grid.GetNumPoints( 0 ) ;
        const int npy   = grid.GetNumPoints( 1 ) ;
        const int npz   = grid.GetNumPoints( 2 ) ;
        const int npxy  = npx * npy ;
...
/* static */ void InteSiVis::GlutDisplayCallback(void)
{
    ...
    {   // This is the first time this app has displayed anything.
        ...
        // Light & heavy vorton textures
        {
            ...
            sInstance->mVortonMaterial.AssignTexturePage( QdMaterial::TP_NOISE_BALL , 0.5f , 1 , opaqueLight ) ;
            sInstance->mVortonMaterial.AssignTexturePage( QdMaterial::TP_NOISE_BALL , 0.5f , 0 , opaqueHeavy ) ;
    ...
    switch( sInstance->mScenario )
    {
        ...
        case 0: case 1:
            const Vec3      vCoM        = Particles::ComputeCenterOfMass( sInstance->mTracers ) ;
...
/* static */ void InteSiVis::GlutIdleCallback(void)
{
    ...
    {   // Make all vortons have the same size and mass.
        ...
        sInstance->mPclOpEmitVortons.mTemplate.mDensity = sInstance->mPclOpVortonSim.mVortonSim.GetVortons()[0].mDensity ;
    }
        ...
        case 'C':
            {
                ...
                {   // Camera animation is now on.
                    ...
                    {   // Rotate azimuth faster
                        cam.SetOrbitalTrajectory( Vec3( -0.01f , -0.002f , 0.0f ) ) ;
                    }
                    else
