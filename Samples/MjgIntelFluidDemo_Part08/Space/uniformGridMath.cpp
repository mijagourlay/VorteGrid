    ...
    */
    class UniformGrid_StepTowardVectorPoissonSolution_TBB
    {
        ...
        public:
            ...
            UniformGrid_StepTowardVectorPoissonSolution_TBB( UniformGrid< Vec3 > & solution , const UniformGrid< Vec3 > & laplacian , GaussSeidelPortion redOrBlack )
                : mSolution( solution ) , mLaplacian( laplacian ) , mRedOrBlack( redOrBlack ) {}
    } ;

    /*! \brief Function object to compute gradient of the interior of a grid, using Threading Building Blocks
    */
    ...
    for( index[2] = izStart ; index[2] < izEnd ; ++ index[2] )
    {
        ...
        for( index[1] = 1 ; index[1] < dimsMinus1[1] ; ++ index[1] )
        {
            ...
            for( index[0] = 1 ; index[0] < dimsMinus1[0] ; ++ index[0] )
            {
                ...
                /* Compute d/dx */
                rVector.x = ( val[ offsetXPY0Z0 ] - val[ offsetXMY0Z0 ] ) * halfReciprocalSpacing.x ;
                /* Compute d/dy */
                rVector.y = ( val[ offsetX0YPZ0 ] - val[ offsetX0YMZ0 ] ) * halfReciprocalSpacing.y ;
                /* Compute d/dz */
                rVector.z = ( val[ offsetX0Y0ZP ] - val[ offsetX0Y0ZM ] ) * halfReciprocalSpacing.z ;
            }
        }
    }
}
...
/*! \brief Find magnitude min and max of a vector field
...
*/
void ComputeGradient( UniformGrid< Vec3 > & gradient , const UniformGrid< float > & scalarVals )
{
    ...
    // Compute derivatives for interior (i.e. away from boundaries).
    #if USE_TBB
        {
            ...
            parallel_for( tbb::blocked_range<size_t>( 1 , dimsMinus1[2] , grainSize ) , UniformGrid_ComputeGradientInterior_TBB( gradient , scalarVals ) ) ;
        }
    #else
...
#define COMPUTE_FINITE_PARTIAL_DIFFERENTIALS_AT_BOUNDARIES                                                                                          \
    Vec3 & rVector = gradient[ offsetX0Y0Z0 ] ;                                                                                                     \
    if     ( index[0] == 0             )    { rVector.x = ( scalarVals[ offsetXPY0Z0 ] - scalarVals[ offsetX0Y0Z0 ] ) * reciprocalSpacing.x ;     } \
    else if( index[0] == dimsMinus1[0] )    { rVector.x = ( scalarVals[ offsetX0Y0Z0 ] - scalarVals[ offsetXMY0Z0 ] ) * reciprocalSpacing.x ;     } \
    else                                    { rVector.x = ( scalarVals[ offsetXPY0Z0 ] - scalarVals[ offsetXMY0Z0 ] ) * halfReciprocalSpacing.x ; } \
    if     ( index[1] == 0             )    { rVector.y = ( scalarVals[ offsetX0YPZ0 ] - scalarVals[ offsetX0Y0Z0 ] ) * reciprocalSpacing.y ;     } \
    else if( index[1] == dimsMinus1[1] )    { rVector.y = ( scalarVals[ offsetX0Y0Z0 ] - scalarVals[ offsetX0YMZ0 ] ) * reciprocalSpacing.y ;     } \
    else                                    { rVector.y = ( scalarVals[ offsetX0YPZ0 ] - scalarVals[ offsetX0YMZ0 ] ) * halfReciprocalSpacing.y ; } \
    if     ( index[2] == 0             )    { rVector.z = ( scalarVals[ offsetX0Y0ZP ] - scalarVals[ offsetX0Y0Z0 ] ) * reciprocalSpacing.z ;     } \
    else if( index[2] == dimsMinus1[2] )    { rVector.z = ( scalarVals[ offsetX0Y0Z0 ] - scalarVals[ offsetX0Y0ZM ] ) * reciprocalSpacing.z ;     } \
    else                                    { rVector.z = ( scalarVals[ offsetX0Y0ZP ] - scalarVals[ offsetX0Y0ZM ] ) * halfReciprocalSpacing.z ; }
