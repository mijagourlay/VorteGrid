...
unsigned gNumTracerBodyHits = 0 ;
    ...
    */
    class FluidBodySim_CollideTracers_TBB
    {
        ...
        public:
            FluidBodySim_CollideTracers_TBB( Vector< Particle > & rTracers , float ambientFluidDensity , RbSphere & rRbSphere )
                : mTracers( rTracers )
...
/* static */ void FluidBodySim::CollideTracersSlice( Vector< Particle > & particles , float ambientFluidDensity , const RbSphere & rSphere , Vec3 & rImpulseOnBody , const size_t iPclStart , const size_t iPclEnd )
{
    ...
    {   // For each tracer in the simulation...
        ...
        {   // Tracer is colliding with body.
            ...
            const Vec3  vVelDueToRotation   = rSphere.mAngVelocity ^ vDisplacementNew ; // linear velocity, at particle new position, due to body rotation
            ...
            const Vec3  vMinusPclVelChange  = rTracer.mVelocity - rSphere.mVelocity + vVelDueToRotation ;   // (negative of) linear velocity change applied to vorton, minus vVelDueToRotation for Magnus effect
...
/*! \brief Calculate and apply buoyancy on rigid bodies immersed in a fluid

    \param particles - dynamic array of particles

    \param velGrid - uniform grid of velocity values.
            NO LONGER USED.  Instead, the velocity at each "contact point" comes
            directly from the particle velocity.
    ...
    \param bRespectAngularVelocity - This indicates whether a particle-body
            ...
            vortons, without any passive tracers.  But to keep computational
            cost down, the simulation typically has far fewer vortons than
            passive tracers.  To avoid applying extraneous impulses to rigid
            bodies, the simulation treats vortons and tracers separately,
            relegating linear momentum to tracers and angular momentum to
            vortons, when applying impulses to rigid bodies.
...
QUERY_PERFORMANCE_ENTER ;
    ...
    {   // For each body in the simulation...
        ...
        vQueryPos = rSphere.mPosition + 0.5f * rSphere.mRadius * gravityDir ;
        if( densityDeviationGrid.Encompasses( vQueryPos ) )
        {
...
/*! \brief Collide particles with rigid bodies
    ...
    \param ambientFluidDensity - fluid density in the absense
            of fluid particles.  This treatment allows
            particles to represent deviations in ambient
            fluid density, thereby allowing for a more
...
*/
/* static */ void FluidBodySim::SolveBoundaryConditions( Vector< Particle > & particles , const UniformGrid< Vec3 > & velGrid , float ambientFluidDensity , const Vector< RigidBody * > & rigidBodies , bool bRespectAngularVelocity )
{
...
QUERY_PERFORMANCE_ENTER ;
    ...
    {   // For each body in the simulation...
        ...
        {   // Treat particles as vortons.
            ...
            {   // For each vorton in the simulation...
                ...
                {   // Vorton is interacting with body.
                    ...
                    #define FLUID_BODY_CONDUCTION_WITH_CONSTANT_THERMAL_EXPANSION 0
                    #if FLUID_BODY_CONDUCTION_WITH_CONSTANT_THERMAL_EXPANSION
                        // In this case, rSphere.mTemperature has units of density, and assumes the temperature is only used for this calculation.
                        // This code is here to demonstrate how much simpler this calculation can become, but it's not fully fleshed out.
                        const float temperatureDifference       = -rSphere.mTemperature - rVorton.mDensity ;
                    #else
                    ...
                    #endif
                        const float heatConduction              = temperatureDifference * rSphere.mThermalConductivity /* * vortRadius2 */ ;
                        const float heatExchange                = heatConduction /* * timeStep */ ;
                    #if FLUID_BODY_CONDUCTION_WITH_CONSTANT_THERMAL_EXPANSION
                        rVorton.mDensity += heatExchange ;  // Implies heat capacity == 1 for vortons
                    #else
        ...
        {   // Treat particles as non-rotating
            ...
            FluidBodySim_CollideTracers_TBB ct( particles , ambientFluidDensity , rSphere ) ;
