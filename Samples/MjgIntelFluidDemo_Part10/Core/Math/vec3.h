...
#if MEM_ALIGN_16
// SSE -- Streaming SIMD Extensions for x86 architectures
#define USE_SSE 1
#else
#define USE_SSE 0
#endif
...
namespace PeGaSys
{
    namespace Math
    {
        ...
        /*! \brief Plain-old-data struct for Vec3, but that can be in a union.
        */
        struct Vector3
        {
            float x ;   ///< x-component
...
struct Vec3 : public PeGaSys::Math::Vector3
{
    public:
        ...
        //! \brief construct vector from array of floats
        ...
        bool Resembles( const Vec3 & other , const float tolerance = 1.0e-5f ) const
        {
            const Vec3          vDiff       = other - ( * this ) ;
            const float         fDiffMag2   = vDiff.Mag2() ;
            if( fDiffMag2 >= tolerance )
            {   // Absolute difference exceeds tolerance.
                const float fMag2 = this->Mag2() ;
                // Use relative difference.
                return ( fDiffMag2 / fMag2 < tolerance ) ;
            }
            return ( fDiffMag2 < tolerance ) ;
        }
