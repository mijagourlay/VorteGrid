...
static const float sParticleScale = 1.0f ;
...
*/
ParticleRenderer::ParticleRenderer( const char * pParticleData , size_t stride , size_t offsetToAngVel , size_t offsetToSize , size_t offsetToDensity )
...
void ParticleRenderer::FillVertexBufferSlice( const double & timeNow , const Mat4 & viewMatrix , size_t iPclStart , size_t iPclEnd )
{
    ...
    const float v0Heavy     = texCoordForPage0of2( 0.01f ) ;
    const float v1Heavy     = texCoordForPage0of2( 0.99f ) ;
    const float v0Light     = texCoordForPage1of2( 0.01f ) ;
    const float v1Light     = texCoordForPage1of2( 0.99f ) ;
    ...
    if( gVboSupported )
    {
        //if( mVertexBufferGrew ) // We used to assign texture coordinates only if the vertex buffer grew.
        // But now that density varies over time, and since density determines texture coordinates,
        // this needs to get assigned every frame.
        ...
        {   // For each particle in this slice...
            ...
            const float     cosAngle    = cos( pclAngle ) ;
            const float     sinAngle    = sin( pclAngle ) ;
            const float     rSize       = sParticleScale * * ( (float*) pSize ) ;
    ...
    #endif
    {
        ...
        {   // For each particle in this slice...
            ...
            // Assign which texture page to use based on sign density variation.
            const float v0 = rDensity >= 0.0f ? v0Heavy : v0Light ;
            const float v1 = rDensity >= 0.0f ? v1Heavy : v1Light ;
