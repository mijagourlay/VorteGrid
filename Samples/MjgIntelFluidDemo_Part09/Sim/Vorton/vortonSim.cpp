...
#include "vortonSim.h"
    ...
    */
    class VortonSim_DiffuseVorticityPSE_TBB
    {
            float                                       mTimeStep   ;   ///< Duration since last time step.
            VortonSim *                                 mVortonSim  ;   ///< Address of VortonSim object
            const UniformGrid< Vector< unsigned > > &   mUgVortIdx  ;   ///< Reference to uniform grid of vorton indices
        public:
            void operator() ( const tbb::blocked_range<size_t> & r ) const
            {   // Compute subset of vorticity diffusion.
                mVortonSim->DiffuseVorticityPSESlice( mTimeStep , mUgVortIdx , r.begin() , r.end() ) ;
            }
            VortonSim_DiffuseVorticityPSE_TBB( float timeStep , VortonSim * pVortonSim , const UniformGrid< Vector< unsigned > > & ugVortIdx )
                : mTimeStep( timeStep )
                , mVortonSim( pVortonSim )
...
void VortonSim::GenerateBaroclinicVorticitySlice( float timeStep , size_t iPclStart , size_t iPclEnd )
{
    ...
    {   // For each particle in this slice...
        ...
        const Vec3  baroclinicGeneration = densityGradient ^ mGravAccel / GetAmbientDensity() ;
        // The formula for the line below is meant to update vorticity,
        // but since we store that as angular velocity, the formula has an extra factor of 0.5.
...
*/
void VortonSim::DiffuseVorticityPSESlice( const float & timeStep , const UniformGrid< Vector< unsigned > > & ugVortIdx , size_t izStart , size_t izEnd )
{
    ...
    {   // For all points along z except the last...
        ...
        {   // For all points along y except the last...
            ...
            {   // For all points along x except the last...
                ...
                for( unsigned ivHere = 0 ; ivHere < ugVortIdx[ offsetX0Y0Z0 ].Size() ; ++ ivHere )
                {   // For each vorton in this gridcell...
                    const unsigned &    rVortIdxHere    = ugVortIdx[ offsetX0Y0Z0 ][ ivHere ] ;
                    ...
                    // following the "here" vorton.  Thta is because each
                    ...
                    // redundant.
                    for( unsigned ivThere = ivHere + 1 ; ivThere < ugVortIdx[ offsetX0Y0Z0 ].Size() ; ++ ivThere )
                    {   // For each OTHER vorton within this same cell...
                        const unsigned &    rVortIdxThere   = ugVortIdx[ offsetX0Y0Z0 ][ ivThere ] ;
                    ...
                    // vortons in cells in the negative direction.
                    {
                        ...
                        for( unsigned ivThere = 0 ; ivThere < ugVortIdx[ offsetXpY0Z0 ].Size() ; ++ ivThere )
                        {   // For each vorton in the adjacent cell in +X direction...
                            const unsigned &    rVortIdxThere   = ugVortIdx[ offsetXpY0Z0 ][ ivThere ] ;
                            Vorton &            rVortonThere    = mVortons[ rVortIdxThere ] ;
                            Vec3 &              rAngVelThere    = rVortonThere.mAngularVelocity ;
                            const Vec3          vortDiff        = rAngVelHere - rAngVelThere ;
...

                    {
                        ...
                        for( unsigned ivThere = 0 ; ivThere < ugVortIdx[ offsetX0YpZ0 ].Size() ; ++ ivThere )
                        {   // For each vorton in the adjacent cell in +Y direction...
                            const unsigned &    rVortIdxThere   = ugVortIdx[ offsetX0YpZ0 ][ ivThere ] ;
                            Vorton &            rVortonThere    = mVortons[ rVortIdxThere ] ;
                            Vec3 &              rAngVelThere    = rVortonThere.mAngularVelocity ;
                            const Vec3          vortDiff        = rAngVelHere - rAngVelThere ;
                            const Vec3          exchange        = mViscosity * timeStep * vortDiff ;    // Amount of vorticity to exchange between particles.
...

                    {
                        ...
                        for( unsigned ivThere = 0 ; ivThere < ugVortIdx[ offsetX0Y0Zp ].Size() ; ++ ivThere )
                        {   // For each vorton in the adjacent cell in +Z direction...
                            const unsigned &    rVortIdxThere   = ugVortIdx[ offsetX0Y0Zp ][ ivThere ] ;
                            Vorton &            rVortonThere    = mVortons[ rVortIdxThere ] ;
                            Vec3 &              rAngVelThere    = rVortonThere.mAngularVelocity ;
                            const Vec3          vortDiff        = rAngVelHere - rAngVelThere ;
                            const Vec3          exchange        = mViscosity * timeStep * vortDiff ;    // Amount of vorticity to exchange between particles.
...
void VortonSim::DiffuseVorticityPSE( const float & timeStep , const unsigned & uFrame )
{
    ...
    const unsigned   nzm1   = nz - 1 ;
        ...
        parallel_for( tbb::blocked_range<size_t>( 0 , nzm1 , grainSize ) , VortonSim_DiffuseVorticityPSE_TBB( timeStep , this , ugVortIdx ) ) ;
    #else
        DiffuseVorticityPSESlice( timeStep , ugVortIdx , 0 , nzm1 ) ;
    #endif
