...
void Particles::Emit( Vector< Particle > & particles , const UniformGridGeometry & uniformGrid , unsigned multiplier , const Vector< Particle > * vortons )
{
    ...
    if( vortons )
    {
        ...
        {   // For each particle in the array...
            ...
            densityDeviationGrid.Accumulate( rPosition , rParticle.GetMass() ) ;
        }
    ...
    const Vec3      jitterMagnitude = 2.0f * sOneMinusEpsilon * vShift ;
...
void Particles::Advect( Vector< Particle > & particles , const UniformGrid< Vec3 > & velocityGrid , const float & timeStep , const unsigned & uFrame )
{
    ...
    if( velocityGrid.GetExtent() == Vec3( 0.0f , 0.0f , 0.0f ) )
