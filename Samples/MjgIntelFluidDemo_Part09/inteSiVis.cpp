    ...
    , mDecorations( DECO_NONE )
{
...

    {
        ...
        {
            // Note: If mParticleGroupVortons created its own particle vector,
            // mPclOpVortonSim would not need to make its own.  The fact that it
            // does is unusual among PclOps.
            Vector< Particle > * const pVortons = (Vector< Particle > * ) & mPclOpVortonSim.mVortonSim.GetVortons() ;

            mParticleGroupVortons.mParticles = pVortons ;
        ...
        {
            mParticleGroupTracers.mParticles = & mTracers ;
...
void InteSiVis::InitialConditions( unsigned ic )
{
    ...
    {   // Switch on initial conditions
        ...
        break ;
        case 2: // Projectile
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 1024 , VortexNoise( Vec3( 4.0f * fThickness , 0.3f * fThickness , 0.3f * fThickness ) ) ) ;
            // Add a sphere
            GetSpheres().PushBack( RbSphere( Vec3( -3.0f , 0.0f , 0.0f ) , Vec3( 10.0f , 0.0f , 0.0f ) , 0.02f , 0.2f ) ) ;
        ...
        case 3: // Projectile with spin about longitudinal axis
            AssignVortons( vortons , 0.125f * FLT_EPSILON , 2048 , VortexNoise( Vec3( 4.0f * fThickness , 0.3f * fThickness , 0.3f * fThickness ) ) ) ;
            // Add a sphere
            GetSpheres().PushBack( RbSphere( Vec3( -2.0f , 0.0f , 0.0f ) , Vec3( 10.0f , 0.0f , 0.0f ) , 0.02f , 0.2f ) ) ;
            GetSpheres()[0].ApplyImpulsiveTorque( Vec3( 0.01f , 0.0f , 0.0f ) ) ; // Make sphere spin
            mCamera.SetTarget( Vec3( 1.0f , 0.0f , 0.0f ) ) ;
            mCamera.SetEye( Vec3( 1.0f , -3.0f , 0.0f ) ) ;
            numTracersPerCellCubeRoot = 6 ;
        break ;
        ...
        case 13: // Two balls of initially stationary fluid, one heavy, one light, heavy one above light one.
            AssignVortons( vortons , 1.0f , numVortonsMax , DensityBall( fRadius , -1.0f ) , Vec3( 0.0f , 0.0f , -2.0f ) ) ;
            AssignVortons( vortons , 1.0f , numVortonsMax , DensityBall( fRadius ,  1.0f ) , Vec3( 0.0f , 0.0f ,  2.0f ) ) ;
            mCamera.SetTarget( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
            ...
            vortonsImplyingTracers = ( const Vector< Particle > * )( & vortons ) ;
        break ;
        case 14: // Two boxes of initially stationary fluid, one heavy, one light, light one above heave one.
        {
            AssignVortons( vortons , 1.0f , numVortonsMax / 2 , DensityBox( Vec3( fRadius , fRadius , fRadius ) ,  1.0f ) , Vec3( 0.0f , 0.0f , -0.5f ) ) ;
            AssignVortons( vortons , 1.0f , numVortonsMax / 2 , DensityBox( Vec3( fRadius , fRadius , fRadius ) , -1.0f ) , Vec3( 0.0f , 0.0f ,  0.5f ) ) ;
    ...
    Particles::Emit( mTracers , vortonSim.GetGrid() , numTracersPerCellCubeRoot , vortonsImplyingTracers ) ;
...
{
    if( grid.GetExtent() == Vec3( 0.0f , 0.0f , 0.0f ) )
    {   // Grid has no size.
