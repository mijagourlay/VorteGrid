...
// DO NOT SUBMIT
static const Vorton * vorton0 ;
static const Vec3 * angVel0 ;
static const float * angVelX0 ;
...
#if ! ( COMPUTE_DENSITY_AND_GRADIENT_AT_AND_WITH_VORTONS_USING_SPH || COMPUTE_DENSITY_GRADIENT_AT_AND_WITH_VORTONS ) // This variety does not yet have a TBB implementation.
...
// TODO: Remove mSignedDistanceGrid and this call from VortonSim.  This low-res version is only used to initialize tracers and then subsequently not assigned from vortons.
PopulateSignedDistanceGridFromParticleDensity( mSignedDistanceGrid , mDensityGrid , 0 ) ;
...
void VortonSim::FindBoundingBox()
{
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_FindBoundingBox_Vortons ) ;
}
    ...
    {   // This is the final amendment to the bounding box.
        ...
        #define JITTER_BOX 0
            ...
            {   // Using differential velocity-from-vorticity technique.
                ...
                //
                // TODO: FIXME: Instead of expanding boundaries, use treecode or direct summation to
                // compute velocity-from-vorticity at domain boundaries, then use those as boundary
...
void VortonSim::MakeBaseVortonGrid( NestedGrid< Vorton > & influenceTree )
{
    ...
    ASSERT( 0 ) ;   // Poisson and direct solvers do not use this routine.
    return ;
#else
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_MakeBaseVortonGrid ) ;
}
...
*/
void VortonSim::ComputeVelocityAtGridpoints_Slice( unsigned izStart , unsigned izEnd , const UniformGrid< VECTOR< unsigned > > & vortonIndicesGrid , const NestedGrid< Vorton > & influenceTree )
{
...
void VortonSim::ComputeVelocityAtVortons_Slice( unsigned iPclStart , unsigned iPclEnd , const UniformGrid< VECTOR< unsigned > > & vortonIndicesGrid , const NestedGrid< Vorton > & influenceTree )
{
    ...
    {   // For subset of vortex particle index values...
        ...
        #endif
    #elif VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_MONOPOLES
        ASSERT( 0 ) ;  // Monopole method does not use this routine.
        (void) vPosition , vVelocity ; // Avoid "local variable is initialized but not referenced" warning.
    #elif VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_POISSON_GAUSS_SEIDEL
        ASSERT( 0 ) ; // Differential solvers do not use this routine.
        (void) vPosition , vVelocity ; // Avoid "local variable is initialized but not referenced" warning.
...
void VortonSim::PopulateVorticityGridFromVortons( UniformGrid< Vec3 > & vorticityGrid , float scale )
{
...
#if defined( _DEBUG )
    {
        ...
        ASSERT( vCirculationVortons.Resembles( scale * vCirculationGrid , 0.001f ) ) ;
    }
#endif
...
void VortonSim::ComputeVelocityFromVorticity_Integral( const UniformGrid< VECTOR< unsigned > > & vortonIndicesGrid , const NestedGrid< Vorton > & influenceTree )
{
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_ComputeVelocityFromVorticity_Integral ) ;
}
...
*/
void VortonSim::ComputeVelocityFromVorticity_Differential( const UniformGrid< Vec3 > & vorticityGrid )
{
    QUERY_PERFORMANCE_ENTER ;
    ...
    QUERY_PERFORMANCE_ENTER ;
    {
            ...
            #else   // Not really multi-grid. Just uses multi-grid data structures.  For diagnosing data flow.
                vectorPotentialMultiGrid[0].Init( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
                SolveVectorPoisson( vectorPotentialMultiGrid[0] , mVorticityMultiGrid[ 0 ] ) ;
                #if 0 // Ultra-specific diagnostic.
                {
                    UniformGrid< Vec3 > vectorPotentialDummy( mGridTemplate ) ;
                    vectorPotentialDummy.Init( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
                    SolveVectorPoisson( vectorPotentialDummy , vorticityGrid ) ;
                    ASSERT( vorticityGrid == mVorticityMultiGrid[0] ) ;
                    ASSERT( vectorPotentialDummy == vectorPotentialMultiGrid[0] ) ;
                }
                #endif
            #endif
            (void) vorticityGrid ; // Avoid "unused formal parameter" compiler warning.
        #else
            vectorPotential.Init( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
            SolveVectorPoisson( vectorPotential , vorticityGrid ) ;
        #endif
    }
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_ComputeVelocityFromVorticity_Differential ) ;
}
...
/** Compute velocity due to vortons, for every point in a uniform grid.
    ...
    \note This routine assumes CreateInfluenceTree has already executed.
*/
void VortonSim::ComputeVelocityFromVorticity( const UniformGrid< VECTOR< unsigned > > & vortonIndicesGrid , const NestedGrid< Vorton > & influenceTree , const UniformGrid< Vec3 > & vorticityGrid )
{
    QUERY_PERFORMANCE_ENTER ;
    ...
    (void) vorticityGrid ; // Avoid "unused formal parameter" compiler warning.
#elif ( ( VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_POISSON_GAUSS_SEIDEL ) )
    ComputeVelocityFromVorticity_Differential( vorticityGrid ) ;
    (void) influenceTree , vortonIndicesGrid ; // Avoid "unused formal parameter" compiler warning.
#else
    #error Velocity technique is invalid or undefined.  Assign VELOCITY_TECHNIQUE in vorton.h or change this code.
#endif

    QUERY_PERFORMANCE_EXIT( VortonSim_ComputeVelocityFromVorticity ) ;
}
...
void VortonSim::PopulateDensityAndMassFractionGrids( UniformGrid< float > & densityGrid , const VECTOR< Particle > & particles , const unsigned uFrame )
{
    ...
    UniformGrid< float > ugParticleContribution( static_cast< UniformGridGeometry >( densityGrid ) ) ;
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_PopulateDensityAndMassFractionGrids ) ;
}
...
static void PoisonDensityGradient( UniformGrid< Vec3 > & densityGradientGrid , const VECTOR< Vorton > & particles )
{
...
#if USE_TBB && 0 // Thread-safe multi-threaded version runs slower than serial vcersion, so this is disabled.
    // Estimate grain size based on size of problem and number of processors.
    const size_t grainSize =  Max2( 1 , numVortons / gNumberOfProcessors ) ;
    // Compute velocity at gridpoints using multiple threads.
    ...
    QUERY_PERFORMANCE_EXIT( PoisonDensityGradient_VortonHits ) ;
}
...
static void PoisonDensityGradient( UniformGrid< Vec3 > & densityGradientGrid , UniformGrid< float > & densityGrid )
{
    ...
    QUERY_PERFORMANCE_EXIT( PoisonDensityGradient_SphereGeometry ) ;
}
#endif
...
inline void VortonSim::CombustAndSetMassFractions( float timeStep , Vorton & rVorton )
{
    ...
    //QUERY_PERFORMANCE_EXIT( VortonSim_CombustAndSetMassFractions ) ;
...
*/
void VortonSim::PopulateSignedDistanceGridFromParticleDensity( UniformGrid< float > & signedDistanceGrid , const UniformGrid< float > & densityGrid , const unsigned uFrame )
{
    QUERY_PERFORMANCE_ENTER ;
...
//ComputeSDF_Sphere( signedDistanceGrid ) ; // DO NOT SUBMIT. Diagnosing surface tracer placment
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_PopulateSignedDistanceGrid ) ;
}
                                            ...
                                            )
{
    ...
    #if ENABLE_FLUID_BODY_SIMULATION
        #if USE_TBB && 0 // TBB version of this not yet implemented
            // Estimate grain size based on size of problem and number of processors.
            const size_t grainSize =  Max2( 1 , numVortons / gNumberOfProcessors ) ;
...
*/
void VortonSim::UpdateVpm( float timeStep , unsigned uFrame )
{
    QUERY_PERFORMANCE_ENTER ;
    ...
    // Grid populated with vorticity from vortons.
    // This is only used in certain techniques, such as particle-in-cell, or Poisson.
    UniformGrid< Vec3 > vorticityGrid ;

#if USE_PARTICLE_IN_CELL
    #if VELOCITY_TECHNIQUE_POISSON_GAUSS_SEIDEL
        #error USE_PARTICLE_IN_CELL cannot work with VELOCITY_TECHNIQUE_POISSON_GAUSS_SEIDEL.
    #endif
    PopulateVorticityGridFromVortons( vorticityGrid , 1.0f ) ;
    ...
    AssignVortonsFromVorticity( vorticityGrid ) ;
#endif
...
//#error TODO: Use mVelFromVortTechnique to change VELOCITY_TECHNIQUE to a runtime decision.
    ...
    CreateInfluenceTree( influenceTree ) ;
        ...
        #if 0
        {   // Ultra-specific diagnostic that only applies for the "#else" case "not really multi-grid" in ComputeVelocityFromVorticity.
            PopulateVorticityGridFromVortons( vorticityGrid , -1.0f ) ;
            ASSERT( vorticityGrid == mVorticityMultiGrid[ 0 ] ) ;
        }
        #endif
    ...
    ComputeVelocityFromVorticity( mVortonIndicesGrid , influenceTree , vorticityGrid ) ;
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_UpdateVpm ) ;
}
...
void VortonSim::Update( float timeStep , unsigned uFrame )
{
    ...
    {   // Use smoothed particle hydrodynamics.
        UpdateSph( timeStep , uFrame ) ;
    }
    else if( FLUID_SIM_VPM_SPH_HYBRID == mFluidSimTechnique )
    {   // Use vortex particle method.
        UpdateVpm( timeStep , uFrame ) ;
//#error READ ALL COMMENTS before proceeding.
//#error This call to UpdateSph doesn't do what I want, but it also does more than I meant.
...
//#error TODO: Comment out everything in UpdateSph except ApplyBodyForce, and make that routine not use accelerations (or make accelerations all zero).
        ...
        // I meant for UpdateVpm to assign vel from vort, and UPdateSph to reduce divergence and accel grav.
        //
...
//#error TODO: Need to try the selective Baro/linear idea I described in Evernote.
        ...
        // The solution should entail either moving that transfer into UpdateVpm & UpdateSph, or alternating VPM and SPH phases,
    ...
    QUERY_PERFORMANCE_EXIT( VortonSim_Update ) ;
}
