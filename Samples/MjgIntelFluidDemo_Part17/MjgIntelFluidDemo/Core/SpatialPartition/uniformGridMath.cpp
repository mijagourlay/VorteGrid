...
#define NATURAL_BOUNDARY_CONDITION 1
    ...
    #define SOLVE_POISSON_AT_BOUNDARIES                                                 \
        ...
        size_t idxProxy[3] = {  Clamp( index[0] , size_t( 1 ) , dimsMinus1[0] - 1 )     \
        ...
        ASSERT( ! IsNan( rSolution ) && ! IsInf( rSolution ) ) ;                        \
        (void) idxProxy ;
...
void SolveVectorPoisson( UniformGrid< Vec3 > & soln , const UniformGrid< Vec3 > & lap , size_t numSteps )
{
    ...
    for( size_t iter = 0 ; iter < maxIters ; ++ iter )
    {
        ...
        StepTowardVectorPoissonSolution( soln , lap , 0 , numZ , GS_RED   ) ;
        StepTowardVectorPoissonSolution( soln , lap , 0 , numZ , GS_BLACK ) ;
    #elif POISSON_TECHNIQUE == POISSON_TECHNIQUE_GAUSS_SEIDEL
        StepTowardVectorPoissonSolution( soln , lap , 0 , numZ , GS_BOTH  ) ;
    #else
