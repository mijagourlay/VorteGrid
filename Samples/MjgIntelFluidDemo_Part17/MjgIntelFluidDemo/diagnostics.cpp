...
static const Vec4 sColorRampCyan[] =
{   // Cyan
    Vec4( 0.0f , 0.2f , 0.2f , 0.2f ) ,
    Vec4( 0.0f , 0.5f , 0.5f , 0.5f ) ,
    Vec4( 0.0f , 0.8f , 0.8f , 0.8f ) ,
    Vec4( 0.0f , 0.9f , 0.9f , 0.9f ) ,
    Vec4( 0.0f , 1.0f , 1.0f , 1.0f ) ,
} ;
static const int NUM_COLORS_IN_RAMP_CYAN = sizeof( sColorRampCyan ) / sizeof( sColorRampCyan[0] ) ;


static const Vec4 sColorRampRainbow[] =
{   // Rainbow
...
static const int NUM_COLORS_IN_RAMP_RAINBOW = sizeof( sColorRampRainbow ) / sizeof( sColorRampRainbow[0] ) ;

static const Vec4 sColorRampRGBYMC[] =
...
static const int NUM_COLORS_IN_RAMP_RGBYMC = sizeof( sColorRampRGBYMC ) / sizeof( sColorRampRGBYMC[0] ) ;



static const Vec4 sColorRampMagentaGreen[] =
...
static const int NUM_COLORS_IN_RAMP_MAGENTA_GREEN = sizeof( sColorRampMagentaGreen ) / sizeof( sColorRampMagentaGreen[0] ) ;




static Vec4 const * sColorRamp          = sColorRampCyan ;
static int          sNumColorsInRamp    = NUM_COLORS_IN_RAMP_CYAN ;
...
{
    if( sColorRampCyan == sColorRamp )
    {
        sColorRamp       = sColorRampRainbow ;
        sNumColorsInRamp = NUM_COLORS_IN_RAMP_RAINBOW ;
    }
    else if( sColorRampRainbow == sColorRamp )
    {
        sColorRamp       = sColorRampRGBYMC ;
        sNumColorsInRamp = NUM_COLORS_IN_RAMP_RGBYMC ;
    }
    else if( sColorRampRGBYMC == sColorRamp )
    {
        sColorRamp       = sColorRampMagentaGreen ;
        sNumColorsInRamp = NUM_COLORS_IN_RAMP_MAGENTA_GREEN ;
    }
    else if( sColorRampMagentaGreen == sColorRamp )
    {
        sColorRamp       = sColorRampCyan ;
        sNumColorsInRamp = NUM_COLORS_IN_RAMP_CYAN ;
    }
}
...
static inline Vec4 GetColorFromRamp( const Vec4 * colorRamp , int numColorsInRamp , float zeroToOne )
{
    ...
    ASSERT( ( idxMin >= 0 ) && ( idxMin < numColorsInRamp - 1 ) ) ;
    // Interpolate between adjacent colors within the ramp.
    const int           idxMax                  = idxMin + 1 ;
    ASSERT( ( idxMax >= 1 ) && ( idxMax < numColorsInRamp ) ) ;
...
{
    Vec4 color = GetColorFromRamp( sColorRamp , sNumColorsInRamp , val0to1 ) ; // Color based on magnitude.
...
/** Draw grid cells, given a grid of scalar values.
    ...
    \see DrawGridCells_VectorMagnitude
*/
//#error TODO: Support different line widths.
...
static void DrawGridPoints_Vectors( const UniformGrid<Vec3> & grid , const QdMaterial & material , InteSiVis::GridDecorationsE gridDecorations , bool bRenderDiagnosticText )
{
    ...
    const float lineWidth = InteSiVis::GRID_DECO_POINTS == gridDecorations ? 1.0f : 2.0f ; // thinner for just points, thicker for points and cells.
    const float pointSize = InteSiVis::GRID_DECO_POINTS == gridDecorations ? 2.0f : 4.0f ; // thinner for just points, thicker for points and cells.
    ...
    {   // For each vorton...
        ...
        {   // Vorton is still inside grid.
            ...
            case VORTON_PROPERTY_VELOCITY:
                ...
                color = GetColorFromRamp( sColorRampRainbow , NUM_COLORS_IN_RAMP_RAINBOW , val0to1 ) ;
...
*/
void InteSiVis::RenderDiagnosticPathlines( VECTOR< Particle > & particles )
{
    QUERY_PERFORMANCE_ENTER ;
...
void InteSiVis::RenderVortonDiagnosticVectors()
{
    ...
    ASSERT( ( mVortonRendering == VORTON_RENDER_VECTORS ) || ( mVortonRendering == VORTON_RENDER_PARTICLES_AND_VECTORS ) || ( mVortonRendering == VORTON_RENDER_ALL ) ) ;
...
*/
void InteSiVis::RenderDiagnosticGrid()
{
    QUERY_PERFORMANCE_ENTER ;

    QUERY_PERFORMANCE_ENTER ;
    QdLight::DisableLights() ;
    QUERY_PERFORMANCE_EXIT( InteSiVis_RenderDiagnosticGrid_DisableLights ) ;
    ...
    {   // Grid decorations are enabled.
        ...
        {
            DrawGrid_Vectors( mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetVelocityGrid() , mBoundingBoxMaterial , mGridDecorations , bRenderDiagnosticText ) ;
        }
        else if( GRID_FIELD_DENSITY_GRADIENT == mGridField )
        {
            DrawGrid_Vectors( mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetDensityGradientGrid() , mBoundingBoxMaterial , mGridDecorations , bRenderDiagnosticText ) ;
        }
#if COMPUTE_PRESSURE_GRADIENT
        else if( GRID_FIELD_PRESSURE_GRADIENT == mGridField )
        {
            DrawGrid_Vectors( mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetPressureGradientGrid() , mBoundingBoxMaterial , mGridDecorations , bRenderDiagnosticText ) ;
        }
#endif
        else if( GRID_FIELD_DENSITY == mGridField )
        {
            DrawGrid_Scalars( mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetDensityGrid() , mBoundingBoxMaterial , mGridDecorations , bRenderDiagnosticText ) ;
        }
        else if( GRID_FIELD_SIGNED_DISTANCE == mGridField )
        {
            DrawGrid_Scalars( mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetSignedDistanceGrid() , mBoundingBoxMaterial , mGridDecorations , bRenderDiagnosticText ) ;
        }
    }

    QUERY_PERFORMANCE_EXIT( InteSiVis_RenderDiagnosticGrid_BoundingBox ) ;
}
...
void InteSiVis::RenderSummaryDiagnosticText()
{
    ...
    {   // Diagnostic text is enabled.
        ...
        }
        {
            ...
            oglRenderString( Vec3( 10.0f , 220.0f , 0.0f ) , GLUT_BITMAP_HELVETICA_10 , "diplacementMax=%g numIters=%i" , gVortonSim_DisplacementMax , gVortonSim_NumRelaxationIters ) ;
        }
        // Render tracer statistics.
        {
            oglRenderString( Vec3( 10.0f , 230.0f , 0.0f ) , GLUT_BITMAP_HELVETICA_10 , "tracers: #=%i"
        ...
        oglRenderString( Vec3( 10.0f , 240.0f , 0.0f ) , GLUT_BITMAP_HELVETICA_10 , "%s    %s    %s    %s    %s    %s"
            ...
            , VortonRenderingString()
            , VortonPropertyString()
            , TracerRenderingString() ) ;

        // Render fluid simulation technique
        oglRenderString( Vec3( 10.0f , 250.0f , 0.0f ) , GLUT_BITMAP_HELVETICA_10 , "%s"
...
const char * InteSiVis::VortonRenderingString() const
{
...
const char * InteSiVis::TracerRenderingString() const
{
