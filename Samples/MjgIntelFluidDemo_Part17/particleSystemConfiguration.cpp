...
static ParticleGroup * CreateVortonParticleSystem( FluidVortonPclGrpInfo & vortonPclGrpInfo , const float viscosity , const float ambientFluidDensity , const float fluidSpecificHeatCapacity , const Vec3 & gravityAcceleration , VECTOR< Impulsion::PhysicalObject * > & physicalObjects )
{
    ...
    // NOTE that UpdateVpm and UpdateSph both populate a velocity grid anyway, which is proper, but also apparently redundant with this operation.
...
tracerPclGrpInfo.mPclOpSeedSurfaceTracers->mSignedDistanceGrid  = & vortonPclGrpInfo.mPclOpVortonSim->mVortonSim.GetSignedDistanceGrid() ;
...
ParticleSystem * CreateNonFluidParticleSystem( size_t i )
{
...

    {
        ...
        }
        {
            ...
            emitterTemplate.mPosition               = Vec3( gap - 10.0f , 0.0f , 0.0f ) ;
