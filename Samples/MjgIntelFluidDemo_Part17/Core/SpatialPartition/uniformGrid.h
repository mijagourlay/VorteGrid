...
class UniformGridGeometry
{
    public:
        ...
        */
        void Expand( const UniformGridGeometry & src , int iExpansion[ 3 ] )
        {
            FAIL() ; // Not tested.
            mGridExtent         = src.mGridExtent ;
            mMinCorner          = src.mMinCorner ;
            mNumPoints[ 0 ]     = src.GetNumCells( 0 ) * iExpansion[ 0 ] + 1 ;
            mNumPoints[ 1 ]     = src.GetNumCells( 1 ) * iExpansion[ 1 ] + 1 ;
            mNumPoints[ 2 ]     = src.GetNumCells( 2 ) * iExpansion[ 2 ] + 1 ;
            PrecomputeSpacing() ;
        }
