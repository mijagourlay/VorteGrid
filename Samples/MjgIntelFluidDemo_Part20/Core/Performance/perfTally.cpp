...
// #pragma optimize( "" , off ) // Diagnosing issue in PROFILE build
...
#pragma warning( disable : 4996 ) // This function may be unsafe (sprintf)

#define PANIC( condition ) if( ! ( condition ) ) { int * crash = NULLPTR ; * crash = 0xbada110c ; }

// Types -----------------------------------------------------------------------

// Private variables -----------------------------------------------------------

void * SpinLock::sTlsAddressIsThreadId = NULLPTR ; // Address is used as a unique identifier for each thread.  Actual value of variable does not matter; only its address is used.

// Allocate PerfTally objects linearly to reduce profiling overhead.
static const int    MAX_NUM_PERF_TALLIES    = 2048                  ;   ///< Maximum number of PerfTally objects for entire process.
...
SpinLock            sPerfTalliesLock                                ;   ///< Mutex for synchronizing access to sPerfTallies and sPerfTallyCount.
...
/** Compare 2 strings except treat slashes and backslashes as the same.
*/
static int strCmpUnifySlashes( const char * s1 , const char * s2 )
{
    for( int i = 0 ; s1[ i ] != '\0' && s2[ i ] != '\0' ; ++ i )
    {
        if( s1[ i ] != s2[ i ] )
        {   // Characters do not match.
            if( NotSlash( s1[ i ] ) || NotSlash( s2[ i ] ) )
            {
                return i + 1 ;
            }
        }
...
PerfAggregatedStatsWithId * PerfAggregatedStatsWithId::Find( const PerfBlockIdentifier * id )
{
    ...
    {   // For each PerfAggregatedStats in global pool...
        ...
        {   // Found object with sought label.  That should uniquely identify block but only if labels are globally unique across process.
            ...
            {   // Found sought object.
                if( strCmpUnifySlashes( tuple->mId->mFilename , id->mFilename ) )
                {   // Label is not unique.  Compiler probably used different paths for the same source file.  Maybe #include is not consistent.
                    DEBUG_BREAK() ;
                }
                return tuple ;
            }
...
PerfAggregatedStatsWithId * PerfAggregatedStatsWithId::New()
{
    ...
    PANIC ( sPerfAggregatedStatsCount < MAX_NUM_PERF_AGG_STATS ) ;
...
PerfTally * PerfTally::New( const char * label , const char * filename , unsigned line )
{
    ...
    PANIC ( sPerfTallyCount < MAX_NUM_PERF_TALLIES ) ;
...
*/
void PerfTally::LogHeader( PerfLogFunc logFunc , PerfTally::PerfLogFormat perfLogFormat )
{
    ASSERT( logFunc ) ;

    static const char * tableHeader = "myOrder,callerOrder,IndentedLabel,Filename,Line,Label,Depth,"
    ...
    static const char * callGraphHeader = "digraph {\n" ;

    const char * header = ( PERF_LOG_FORMAT_TABLE == perfLogFormat ) ? tableHeader : callGraphHeader ;
...
/** Log footer (tail text) for performance data.
*/
void PerfTally::LogFooter( PerfLogFunc logFunc , PerfTally::PerfLogFormat perfLogFormat )
{
    ASSERT( logFunc ) ;

    static const char * tableFooter = "\n" ;
    static const char * callGraphFooter = "}\n" ;

    const char * footer = ( PERF_LOG_FORMAT_TABLE == perfLogFormat ) ? tableFooter : callGraphFooter ;
...
/** Log performance statistics for this block and its callees.
    ...
    \param callerOrder  Order (unique ordered key) of caller perf entry.
    
    \param myOrder      Order (unique ordered key) of this perf entry.

    \param indent       Number of layers deep this perf entry is in the callstack.

    \param logFunc      Printf-like function to use for logging output.

    \return Total count of perf entries visited so far.

    Note that times reported here are in milliseconds, whereas durations recorded in variables are in seconds.

*/
size_t PerfTally::Log(
      size_t callerOrder
    , size_t myOrder
    , const int indent
    , const PerfBlockIdentifier * callerId
    , PerfLogFunc logFunc
    , PerfTally::PerfLogFormat perfLogFormat
    )
{
    ASSERT( logFunc != NULL ) ;
    ...
    if( PERF_LOG_FORMAT_TABLE == perfLogFormat )
    {
        // Use comma-separated values, for easy import into a spreadsheet program.
        const char * formatStr = /* myOrder */ "%i," /* callerOrder */ "%i," /* indent-label */ "%s%s," /* filename , line */ "%s,%i," /* label , depth */ "%s,%i,"
    ...
    {
        ASSERT( PERF_LOG_FORMAT_CALL_GRAPH == perfLogFormat ) ;

        // Output call graph info in graphviz format.
        ...
        sprintf( logLine , formatStr
            // Identifier
            , callerId ? callerId->mLabel : "__MAIN__"
            , mId.mLabel
    ...
    size_t calleeOrder = myOrder + 1 ;

    for( PerfTally * callee = mFirstCallee ; callee != 0 ; callee = callee->mNextSiblingCallee )
    {   // For each callee under this block...
        calleeOrder = callee->Log( myOrder , calleeOrder , indent + 1, & mId , logFunc , perfLogFormat ) ;   // Tally stats for that callee.
    }
...
/** Log performance statistics for this block and its callees.
    ...
    \param blockOrder   Order (unique ordered key) of this perf entry.
...
*/
size_t PerfTally::LogThread( size_t blockOrder , PerfLogFunc logFunc , PerfTally::PerfLogFormat perfLogFormat )
{
    ASSERT( logFunc != NULL ) ;

    if( 1 == blockOrder )
    {   // This is the first call to Log so log header info.
        LogHeader( logFunc , perfLogFormat ) ;
    }

    blockOrder = Log( 0 , blockOrder , 0 , NULLPTR , logFunc , perfLogFormat ) ;
