    ...
    for( index[2] = 0 ; index[2] < dims[2] ; ++ index[2] )
    {
        ...
        for( index[1] = 0 ; index[1] < dims[1] ; ++ index[1] )
        {
            ...
            for( index[0] = 0 ; index[0] < dims[0] ; ++ index[0] )
            {
                ...
                ASSERT( ! IsNan( rVal.x ) && ! IsNan( rVal.y ) && ! IsNan( rVal.z ) ) ;
                ASSERT( ! IsInf( rVal.x ) && ! IsInf( rVal.y ) && ! IsInf( rVal.z ) ) ;
...
/* static */ void UniformGrid<unsigned>::UnitTest( )
{
#if defined( _DEBUG )
...

    {
        ...
        DebugPrintf( "dims=%5u , %5u , %5u , cap=%u spacing={ %g , %g , %g }\n" ,
        ...
        // Iterate through each cell in the UniformGrid and obtain its contents.
        {
            size_t idx[3] ;
    ...
    {
        static const Vec3 unitRange( 1.0f , 1.0f , 1.0f ) ; // range of positions
        const Vec3 vMin( 0.0f , 0.0f , 0.0f ) ;   // Minimum coordinate of UniformGrid.
        const Vec3 vMax( vMin + unitRange ) ;   // Maximum coordinate of UniformGrid.
        ...
        vecFuncSolved.Init( Vec3( 0.0f , 0.0f , 0.0f ) ) ;
        SolveVectorPoisson( vecFuncSolved , laplacian , 0 , BC_NEUMANN , residualStats ) ;
    ...
    // Create a UniformGrid with a given domain and manually assigns vector values to each gridpoint.
    {
        ...
        ugm4.Init() ;
        unsigned        idx[3] ;   // Grid cell indices.
        const Vec3 vOffset( vMin + 0.0f * unitRange ) ;
