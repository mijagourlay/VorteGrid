...
template <class ItemT> class NestedGrid
{
    public:
        ...
        */
        ...
        void GetParentIndices( unsigned parentIndices[ 3 ] , const unsigned childIndices[ 3 ] , size_t uLayer )
        {
            parentIndices[ 0 ] = childIndices[ 0 ] / mDecimations[ uLayer ][ 0 ] ;
            parentIndices[ 1 ] = childIndices[ 1 ] / mDecimations[ uLayer ][ 1 ] ;
            parentIndices[ 2 ] = childIndices[ 2 ] / mDecimations[ uLayer ][ 2 ] ;
        }
        ...
        /*! \brief Restrict value from a child layer into a parent layer of the multi-resolution grid
        ...
        */
        void DownSampleInto( unsigned uParentLayer )
        {
            Layer   &   rParentLayer    = operator[]( uParentLayer     ) ;
            Layer   &   rChildLayer     = operator[]( uParentLayer - 1 ) ;
            rParentLayer.DownSample( rChildLayer ) ;
        }


        /*! \brief Interpolate value from a parent layer into a child layer of the multi-resolution grid
        ...
        */
        void UpSampleFrom( unsigned uParentLayer )
        {
            Layer           &       rParentLayer        = operator[]( uParentLayer     ) ;
            Layer           &       rChildLayer         = operator[]( uParentLayer - 1 ) ;
            rChildLayer.UpSample( rParentLayer ) ;
        }
    ...
    private:
        ...
        */
        unsigned PrecomputeNumLayers( const UniformGridGeometry & src )
        {
