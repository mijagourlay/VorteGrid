...
void    VortonSim::ConservedQuantities( Vec3 & vCirculation , Vec3 & vLinearImpulse ) const
{
    ...
    {   // For each vorton in this simulation...
        ...
        const float     volumeElement   = POW3( rVorton.mRadius ) ;
...
void VortonSim::Initialize( unsigned numTracersPerCellCubeRoot )
{
    ...
    mVortonRadius = mVortons[ 0 ].mRadius ;
    ...
    {
        float domainVolume = mGridTemplate.GetExtent().x * mGridTemplate.GetExtent().y * mGridTemplate.GetExtent().z ;
        if( 0.0f == mGridTemplate.GetExtent().z )
        {   // Domain is 2D in XY plane.
            domainVolume = mGridTemplate.GetExtent().x * mGridTemplate.GetExtent().y ;
        }
        const float     totalMass           = domainVolume * mFluidDensity ;
        const unsigned  numTracersPerCell   = POW3( numTracersPerCellCubeRoot ) ;
        mMassPerParticle = totalMass / float( mGridTemplate.GetGridCapacity() * numTracersPerCell ) ;
    }
}
...
void VortonSim::FindBoundingBox( void )
{
    ...
    static const float margin = 2.0f * mVortonRadius ;
...
void VortonSim::MakeBaseVortonGrid( void )
{
    ...
    const float vortonRadius = mVortons[ 0 ].mRadius ;
    ...
    {   // For each vorton in this simulation...
        ...
        ASSERT( 0 ) ; // This routine does not apply to Direct or Gauss methods.
    #else
...
void VortonSim::PopulateNegativeVorticityGridFromVortons( UniformGrid< Vec3 > & vorticityGrid )
{
    ...
    {   // For each vorton in this simulation...
        ...
        Vec3 vCirculationCorrectedVorticity = - rVorton.mVorticity * volumeCorrection ;
...
void VortonSim::AdvectTracers( const float & timeStep , const unsigned & uFrame )
{
    ...
    {
        //UniformGrid< float > densityGrid( mGridTemplate ) ;
        //UniformGrid< float > densityGrid( numTracers , mGridTemplate.GetMinCorner() , mGridTemplate.GetMaxCorner() , true ) ;
        static const count = 256 * 64 * 64 ;
        static const Vec3 vMin( -3.0f , -8.1f , -8.1f ) ;
        static const Vec3 vMax( 29.0f ,  8.1f ,  8.1f ) ;
        UniformGrid< float > densityGrid( count , vMin , vMax , true ) ;

        densityGrid.Init() ;
        const unsigned uCapacity = densityGrid.GetGridCapacity() ;
        for( unsigned uItem = 0 ; uItem < uCapacity ; ++ uItem )
        {   // For each cell in density grid...
            float & rDensity = densityGrid[ uItem ] ;
            rDensity = 0.0f ;   // Initialize the cell to have zero density (i.e. zero mass)
        }
        for( unsigned offset = 0 ; offset < numTracers ; ++ offset )
        {   // For each passive tracer...
            Particle & rTracer = mTracers[ offset ] ;
            // Create grid of density for current timestep.
            densityGrid.Insert( rTracer.mPosition , rTracer.mMass ) ;
        }
        densityGrid.GenerateBrickOfBytes( "density" , uFrame ) ;
    }
#endif
...
{
    const Vec3      vSpacing        = mGridTemplate.GetCellSpacing() ;
    // Must keep tracers away from maximal boundary by at least cell.  Note the +vHalfSpacing in loop.
    const unsigned  begin[3]        = { 1*mGridTemplate.GetNumCells(0)/8 , 1*mGridTemplate.GetNumCells(1)/8 , 1*mGridTemplate.GetNumCells(2)/8 } ;
    const unsigned  end[3]          = { 7*mGridTemplate.GetNumCells(0)/8 , 7*mGridTemplate.GetNumCells(1)/8 , 7*mGridTemplate.GetNumCells(2)/8 } ;
    ...
    {   // For each interior grid cell...
        Vec3 vPosMinCorner ;
        mGridTemplate.PositionFromIndices( vPosMinCorner , idx ) ;
        Particle pcl ;
