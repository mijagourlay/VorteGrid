...
#include "Core/Space/uniformGridMath.h"
...
#if defined( PROFILE )
static const int sFrameCountMax = 6000  ;   // In profile builds, run for a fixed number of frames
    ...
    , mEmphasizeCameraTarget( true )
{
...

    {
        ...
        // Set up particle group for vortons.
        {
            ...
            mPclOpAdvectVortons.mUseVelocityGrid = false ;  // Advect vortons using the velocity assigned to them directly.
        #else
            mPclOpAdvectVortons.mUseVelocityGrid = true ;   // Assign vorton velocity from grid.
        #endif
            ...
            mPclOpFluidBodInteVortons.mPhysicalObjects          = & GetPhysicalObjects() ;
        ...
        // Set up particle group for passive tracers.
        {
            ...
            mPclOpFluidBodInteTracers.mPhysicalObjects      = mPclOpFluidBodInteVortons.mPhysicalObjects ;
...
void InteSiVis::InitialConditions( unsigned ic )
{
    ...
    {   // Switch on initial conditions
        ...
        case 4: // Projectile with spin about transverse axis: curve ball.
            ...
            GetSpheres().Back().GetBody()->ApplyImpulsiveTorque( Vec3( 0.0f , 0.01f , 0.0f ) ) ; // Make sphere spin
        ...
        case 15: // Ball heats fluid
        {
            ...
            // Add a sphere
            GetSpheres().PushBack( RbSphere( Vec3( 0.0f , 0.0f , 0.0f ) , Vec3( 0.0f , 0.0f , 0.0f ) , veryLargeMass , 0.2f ) ) ;
            GetSpheres().Back().GetThermalProperties().SetTemperature( sAmbientTemperature + 300.0f ) ;
        ...
        case 17: // Candle.
        {
            ...
            mPclOpKillAgeVortons.mAgeMax            = 400 ;
            mPclOpKillAgeTracers.mAgeMax            = mPclOpKillAgeVortons.mAgeMax ;
        ...
        case 24: // Box projectile with spin about transverse axis
            ...
            GetBoxes().Back().GetBody()->ApplyImpulsiveTorque( Vec3( 0.0f , 0.001f , 0.0f ) ) ; // Make box spin
    ...
    FluidBodySim::RemoveEmbeddedParticles( mParticleGroupTracers.mParticles , GetPhysicalObjects() ) ;
#endif
    ...
    {   // Render API is initialized.  Initial conditions probably changed (i.e. this is not the first call to this function).
        // Can and should create entities now.
        CreateEntities() ;
    }

#if( ENABLE_FLUID_BODY_SIMULATION )
    FluidBodySim::RemoveEmbeddedParticles( reinterpret_cast< Vector< Particle > & >( * vortonSim.GetVortons() ) , GetPhysicalObjects() ) ;
    FluidBodySim::RemoveEmbeddedParticles( mTracers , GetPhysicalObjects() ) ;
#endif

#if PROFILE
    fprintf( stderr , "Initial condition %i, simulation objects: %i tracers    %i vortons    %i spheres %i boxes\n"
        , ic , mTracers.Size() , vortons.Size() , GetSpheres().Size() , GetBoxes().size() ) ;
#endif
...
void InteSiVis::CreateParticleMaterials()
{
    ...
    // Light & heavy vorton materials.
    {
        ...
        const Vec4 translucentCyanish( 0.1f , 0.9f , 0.9f , 0.125f ) ;
        DecoratedNoiseBallTextureProc noiseBallLight( 1.5f , translucentYellowish ) ;
        DecoratedNoiseBallTextureProc noiseBallHeavy( 1.5f , translucentCyanish ) ;
    ...
    // Light & heavy tracer materials
    {
        ...
        NoiseBallTextureProc noiseBallLight( 0.5f , translucentRedish ) ;
        NoiseBallTextureProc noiseBallHeavy( 0.5f , translucentBluish ) ;
...
void InteSiVis::CreateMaterials()
{
    ...
    mPhysObjMaterial.MakeSimpleNoiseTexture( 256 , 256 , 1 , 0.2f ) ;
    mPhysObjMaterial.SetColor( Vec4( 0.8f , 0.4f , 0.8f , 1.0f ) ) ;

    mBoundingBoxMaterial.MakeSimpleNoiseTexture( 8 , 8 , 1 , 0.0f ) ;
    ...
    mPathlineMaterial.MakeSimpleNoiseTexture( 8 , 8 , 1 , 0.0f ) ;
...
void InteSiVis::CreateEntities()
{
    ...
    {
        mModels.push_back( QdModel( mBallDisplayList , & mPhysObjMaterial ) ) ;
        const float scale = GetSpheres()[ idxSphere ].GetCollisionShape()->GetBoundingSphereRadius() ;
        mModels[ idxSphere ].SetScale( Vec3( scale , scale , scale ) ) ;
        mEntities.push_back( Entity( & GetSpheres()[ idxSphere ] , & mModels[ idxSphere ] ) ) ;
    }

    for( size_t idxBox = 0 ; idxBox < numBoxes ; ++ idxBox , ++ idxPhysObj )
    {
        mModels.push_back( QdModel( mCubeDisplayList , & mPhysObjMaterial ) ) ;
        const Vec3 & dimensions = GetBoxes()[ idxBox ].GetDimensions() ;
        mModels[ idxPhysObj ].SetScale( dimensions ) ;
        mEntities.push_back( Entity( & GetBoxes()[ idxBox ] , & mModels[ idxPhysObj ] ) ) ;
    }
}
...
void InteSiVis::InitializeRendering()
{
	...
	mBallDisplayList = MakeUnitSphere() ;
	mCubeDisplayList = MakeUnitCube() ;
...
void InteSiVis::RenderSky()
{
    ...
    QdLight::DisableLights() ;
    {
        ...
        glPushMatrix() ;
        glTranslatef( 0.0f , 0.0f , 0.0f ) ; // TODO: FIXME: Should translate to camera eye.
...
void InteSiVis::RenderRigidBodies()
{
    ...
    for( size_t idxEntity = 0 ; idxEntity < numEntities ; ++ idxEntity )
    {
        ...
        entity.Update() ;
        entity.mRenderModel->Render( & mLights , mTimeNow ) ;
    }
}
...
*/
void InteSiVis::RenderVortonDiagnosticText()
{
    QUERY_PERFORMANCE_ENTER ;
    ...
    QUERY_PERFORMANCE_EXIT( InteSiVis_Render_RenderVortonDiagnosticText ) ;
}
...
*/
void InteSiVis::RenderVortonDiagnosticVectors()
{
    QUERY_PERFORMANCE_ENTER ;
    ...
    {   // For each vorton...
        ...
        {   // Diagnostic information is opaque enough to render.
            ...
            if( mGridDecorations == GRID_DECO_CELLS )
    ...
    QUERY_PERFORMANCE_EXIT( InteSiVis_Render_RenderVortonDiagnosticVectors ) ;
}
...
*/
void InteSiVis::RenderPathlines( Vector< Particle > & particles )
{
    QUERY_PERFORMANCE_ENTER ;

    mPathlineMaterial.UseMaterial() ;

    //  // Modern STL supports .data() but not older versions like MSVS7.
    ...
    QUERY_PERFORMANCE_EXIT( InteSiVis_Render_RenderPathlines ) ;
}
#endif
...
//    }
    ...
    #if ENABLE_PARTICLE_POSITION_HISTORY
        RenderPathlines( reinterpret_cast< Vector< Particle > & >( * mPclOpVortonSim.mVortonSim.GetVortons() ) ) ;
    #endif
    }
...
void InteSiVis::RenderSummaryDiagnosticText()
{
    ...
    {   // Diagnostic text is enabled.
        ...
        // Render vorticity statistics.
        {
            ...
            mPclOpVortonSim.mVortonSim.GatherVorticityStats( vorticityMin , vorticityMax , vorticityMean , vorticityStdDev , centerOfVorticity ) ;
            oglRenderString( Vec3( 10.0f , 110.0f , 0.0f ) , GLUT_BITMAP_HELVETICA_10 , "omega:[%.0f,%.0f]<%g>+-%-.0f #=%i CoV=%g,%g,%g"
                , vorticityMin , vorticityMax , vorticityMean , vorticityStdDev , mPclOpVortonSim.mVortonSim.GetVortons()->size()
        ...
        {   // For each sphere...
            // Print its temperature.
            const float yPos = 130.0f + float( 10 * ( iBox + GetSpheres().size() ) ) ;
        ...
        // Render jerk statistics.
        {
            ...
            Vector< Particle > & vortonsAsParticles = reinterpret_cast< Vector< Particle > & >( * mPclOpVortonSim.mVortonSim.GetVortons() ) ;
...
void InteSiVis::UpdateRigidBodies()
{
    ...
    if( sInstance->mTimeStepping == PLAY || sInstance->mTimeStepping == SINGLE_STEP )
    {
        PhysicalObject_UpdateSystem( sInstance->GetPhysicalObjects() , sInstance->mTimeStep , sInstance->mFrame ) ;
    }
...
/* static */ void InteSiVis::SpecialKeyHandler( int key , const int modifierKeys , int /* windowRelativeMouseX */ , int /* windowRelativeMouseY */ )
{
    ...
    {   // No modifier keys (Shift, Ctrl or Alt) were pressed.
        switch( key )
        {
            ...
            case GLUT_KEY_F8 : InitialConditions( 12 ) ; break;
            case GLUT_KEY_F9 : InitialConditions( 13 ) ; break;
            case GLUT_KEY_F11: InitialConditions( 14 ) ; break;

            case GLUT_KEY_UP   : mTimeStepping   = PLAY  ; break ;
            case GLUT_KEY_DOWN : mTimeStepping   = PAUSE ; break ;
            case GLUT_KEY_RIGHT: mTimeStep       =  sOneOverThirty ; if( mTimeStepping == PAUSE ) mTimeStepping = SINGLE_STEP ; break ;
            case GLUT_KEY_LEFT : mTimeStep       = -sOneOverThirty ; if( mTimeStepping == PAUSE ) mTimeStepping = SINGLE_STEP ; break ;
...
/* static */ void InteSiVis::GlutKeyboardHandler( unsigned char key , int /* mouseX */ , int /* mouseY */ )
{
    ...
    switch( key )
    {
        ...
        case 'E':
        {
            bool & emphasis = sInstance->mEmphasizeCameraTarget ;
            emphasis = ! emphasis ;
        }
        break ;
        ...
        case 'V':
        {
            VortonRenderingE & vortonRendering = sInstance->mVortonRendering ;
            switch( vortonRendering )
            {
            case VORTON_RENDER_NONE     : vortonRendering = VORTON_RENDER_SIMPLE    ; break ;
            case VORTON_RENDER_SIMPLE   : vortonRendering = VORTON_RENDER_VECTORS   ; break ;
            case VORTON_RENDER_VECTORS  : vortonRendering = VORTON_RENDER_PATHLINES ; break ;
            case VORTON_RENDER_PATHLINES: vortonRendering = VORTON_RENDER_ALL       ; break ;
            case VORTON_RENDER_ALL      : vortonRendering = VORTON_RENDER_NONE      ; break ;
            }
        }
        break ;
...
void InteSiVis::InitDevice( int * pArgc , char ** argv )
{
    ...
    mRenderWindow = glutCreateWindow( "(C) 2009-2012 Michael J. Gourlay: VorteGrid Render Window" ) ;
