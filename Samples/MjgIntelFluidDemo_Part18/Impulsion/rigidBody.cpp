...
void RigidBody::Update( float timeStep )
{
    ...
    ASSERT( ! IsNan( angMomBefore ) ) ;
    mAngularMomentum += mNetTorque * timeStep ;
    ASSERT( ! IsNan( GetAngularMomentum() ) ) ;
...
void RigidBody_UnitTest()
{
    ...
    }
    {
        ...
        for( int i = 0 ; i < numUpdates ; ++ i )
        {
            ...
            ASSERT( body.GetPosition().Resembles( Vec3( distance , 0.0f , 0.0f ) , 1.0e-3f ) ) ;
    ...
    }
    {
        ...
        for( int i = 0 ; i < numUpdates ; ++ i )
        {
            ...
            ASSERT( body.GetOrientation().Resembles( pegasysMatrixRotateX , 1.0e-3f ) ) ;
