...
#include "qdParticleMaterial.h"

#include "qdParticleRenderer.h"
...
#   define VertexFormatFlags_Pos3 (GL_V3F)
    ...
    /// Custom vertex format for position and color.
...
static PFN_GL_GENBUFFERSARB_PROC       glGenBuffers    = 0 ;   ///< VBO Name Generation Procedure
static PFN_GL_MAPBUFFERARB_PROC        glMapBuffer     = 0 ;   ///< VBO Map Buffer procedure
static PFN_GL_UNMAPBUFFERARB_PROC      glUnmapBuffer   = 0 ;   ///< VBO Unmap Buffer procedure
static PFN_GL_BINDBUFFERARB_PROC       glBindBuffer    = 0 ;   ///< VBO Bind Procedure
static PFN_GL_BUFFERDATAARB_PROC       glBufferData    = 0 ;   ///< VBO Data Loading Procedure
static PFN_GL_DELETEBUFFERSARB_PROC    glDeleteBuffers = 0 ;   ///< VBO Deletion Procedure
    ...
    */
    class ParticleRenderer_FillVertexBuffer_TBB
    {
        ...
        public:
            ...
            ParticleRenderer_FillVertexBuffer_TBB( QdParticleRenderer * pParticleRenderer , const double & timeNow , const Mat44 & viewMatrix )
...
*/
QdParticleRenderer::QdParticleRenderer( const char * pParticleData , size_t stride , size_t offsetToAngVel , size_t offsetToSize , size_t offsetToDensityInfo , QdParticleMaterial & material )
...
*/
QdParticleRenderer::~QdParticleRenderer()
{
#if USE_VERTEX_BUFFER_OBJECT
    if( gVboSupported )
...
#   if USE_VERTEX_BUFFER_OBJECT
/** Create empty OpenGL vertex buffer to hold particles for rendering; VBO version
*/
void QdParticleRenderer::CreateVertexBuffer_VBO( double timeNow , unsigned & vertexFormatFlags , unsigned & vertexFormatSize , size_t & numVertices )
{
    ASSERT( numVertices != 0 ) ;
    ASSERT( gVboSupported ) ;
...
/** Create empty OpenGL vertex buffer to hold particles for rendering; non-VBO version
*/
void QdParticleRenderer::CreateVertexBuffer_NonVBO( unsigned & vertexFormatSize , size_t & numVertices )
{
    PERF_BLOCK( ParticleRenderer__CreateVertexBuffer_NonVBO ) ;
...
/** Create empty OpenGL vertex buffer to hold particles for rendering.
*/
void QdParticleRenderer::CreateVertexBuffer( double timeNow , unsigned & vertexFormatFlags , unsigned & vertexFormatSize , size_t numParticles , size_t & numVertices )
{
    PERF_BLOCK( ParticleRenderer__CreateVertexBuffer ) ;

    ASSERT( numParticles != 0 ) ;

    static const unsigned   sNumVerticesPerParticle = 4 ; // number of vertices per particle

    numVertices = numParticles * sNumVerticesPerParticle ;

#if USE_ORIENTED_LIT_PARTICLES
    vertexFormatFlags   = VertexFormatFlags_NormalTexture ;
    vertexFormatSize    = sizeof( VertexFormatPositionNormalTexture ) ;
#else
    vertexFormatFlags   = VertexFormatFlags_Pos3Col4Tex2 ;
    vertexFormatSize    = sizeof( VertexFormatPos3Col4Tex2 ) ;
#endif
    ...
    if( gVboSupported )
    {
        CreateVertexBuffer_VBO( timeNow , vertexFormatFlags , vertexFormatSize , numVertices ) ;
    }
    else
#endif
    {   // Use old-style vertex arrays (not vertex buffer objects)
        UNUSED_PARAM( timeNow ) ;
        CreateVertexBuffer_NonVBO( vertexFormatSize , numVertices ) ;
    }
}
...
{
    QdParticleRenderer::ParticleIndex * pi0 = (QdParticleRenderer::ParticleIndex *) p0 ;
    QdParticleRenderer::ParticleIndex * pi1 = (QdParticleRenderer::ParticleIndex *) p1 ;
...
/** Assign indices used to access particle data and sort them by depth.
    ...
    This is a trick to avoid reordering entire particle within their buffer.
    Instead, this effectively creates a map from index to particle,
    and the particles are then rendered in the corresponding order.

    These indices are NOT for an "index buffer" in the usual computer graphics sense of a buffer of vertex indices, defining triangles.
*/
void QdParticleRenderer::AssignIndicesAndSortParticles( const Vec3 & viewForward , size_t numParticles )
{
    PERF_BLOCK( ParticleRenderer__AssignIndicesAndSortParticles ) ;
...
*/
void QdParticleRenderer::FillVertexBufferSlice_Oriented( const double & timeNow , const Mat44 & viewMatrix , size_t iPclStart , size_t iPclEnd )
{
...
*/
void QdParticleRenderer::FillVertexBufferSlice_SeparateVBOs( const double & timeNow , const Mat44 & viewMatrix , size_t iPclStart , size_t iPclEnd )
{
    ASSERT( gVboSupported ) ;
...
#       endif
    ...
    {   // For each particle in this slice...
        ...
        {   // These are luminous particles.  Vary brightness (not opacity).
            colorMod = Min2( 255 , static_cast< int >( massFraction * mMaterial.GetDensityVisibility() ) ) ;
        }
        else
        {   // These are translucent particles.  Vary opacity (not color).
            ASSERT( mMaterial.GetBlendMode() == QdMaterial::BM_ALPHA ) ;
            alphaMod = Min2( 255 , static_cast< int >( massFraction * mMaterial.GetDensityVisibility() ) ) ;
        }
...
*/
void QdParticleRenderer::FillVertexBufferSlice( const double & timeNow , const Mat44 & viewMatrix , size_t iPclStart , size_t iPclEnd )
{
    static const unsigned   nvpp    = 4 ; // number of vertices per particle
...
#if USE_ORIENTED_LIT_PARTICLES
    FillVertexBufferSlice_Oriented( timeNow , viewMatrix , iPclStart , iPclEnd ) ;
...
#   if USE_SEPARATE_VBOS
    if( gVboSupported )
    {   // Fill multiple separate vertex buffer with "plain" particles without normals
        FillVertexBufferSlice_SeparateVBOs( timeNow , viewMatrix , iPclStart , iPclEnd ) ;
    }
    else
#   endif
...
void QdParticleRenderer::CreateAndFillVertexBuffer( double timeNow , unsigned & vertexFormatFlags , unsigned & vertexFormatSize , size_t numParticles , size_t & numVertices )
{
    PERF_BLOCK( ParticleRenderer__CreateAndFillVertexBuffer ) ;

    ASSERT( numParticles != 0 ) ;

    CreateVertexBuffer( timeNow , vertexFormatFlags , vertexFormatSize , numParticles , numVertices ) ;
    ...
    {
        PERF_BLOCK( ParticleRenderer__CreateAndFillVertexBuffer_FillVertexBufferSlice ) ;
...
*/
void QdParticleRenderer::Render( double timeNow , float /* timeChange */ , size_t numParticles )
{
    PERF_BLOCK( ParticleRenderer__Render ) ;
    ...
    unsigned    vertexFormatFlags   = 0 ;
    unsigned    vertexFormatSize    = 0 ;
    size_t      numVertices         = 0 ;

    CreateAndFillVertexBuffer( timeNow , vertexFormatFlags , vertexFormatSize , numParticles , numVertices ) ;

    // Set material used to render particles.
...
#       else    // Use a single VBO
        {
            ...
            glBindBuffer( GL_ARRAY_BUFFER , mVboNames[ 0 ] ) ;
            // Technically this glUnmapBuffer should occur immediately after filling the vertex data, not just prior to rendering.
            // OpenGL only allows a buffer to be unmapped if it is mapped.  As it happens, the particle vertex buffer is mapped each frame,
            // so it "accidentally" works out to call glUnmappBuffer each frame just prior to rendering, like this, but this is not
            // a call pattern that would work for a vertex buffer that does not change every frame.
