...
class IVorticityDistribution
{
    public:
        ...
        virtual Vec3 GetDomainSize() const = 0 ;
...
class VortexRing : public IVorticityDistribution
{
    public:
        ...
        */
        ...
        /// Return size of domain required to fit this ball.
        virtual Vec3 GetDomainSize() const
        {
        ...
        */
        ...
        virtual void AssignVorton( Vec3 & vorticity , float & density , const Vec3 & position , const Vec3 & vCenter ) const
        {
            ...
            {   // Probe position is inside jet region.
                ...
                Vec3            phiHat              = mDirection ^ rhoHat ;  // direction along annular core
                vorticity                           = vortPhi * phiHat ;
                density = 1.0001f ; // Just enough density change to register for tracer placement
            }
            else
            {
                vorticity = Vec3( 0.0f , 0.0f , 0.0f ) ;
    ...
    public:
        ...
        */
        ...
        virtual void AssignVorton( Vec3 & vorticity , float & density , const Vec3 & position , const Vec3 & vCenter ) const
        {
            ...
            density = 1.0f + FLT_EPSILON ; // Slightly positive so that thermal diffusion doesn't cause a sudden global jump.
        }
...
class DensityBall : public IVorticityDistribution
{
    public:
        /** Initialize ball of fluid with the given density deviation-about-ambient.
            ...
            \param  ballRadius  Radius of ball of fluid.

            \param  densityDeviation    Additive density deviation relative to
                        the ambient density of surrounding fluid.  Note that
                        this is NOT a scale, but gets converted to one internally.
        */
        DensityBall( float ballRadius , float densityDeviation )
            : mRadius( ballRadius )
            , mDensity( 1.0f + densityDeviation ) // Assumes ambient density is 1.
        {
        ...
        float   mDensity    ;   ///< Density of fluid in ball.
} ;




/** Specify a ball of fluid with a given density gradient.
*/
class DensityGradientBall : public IVorticityDistribution
{
    public:
        /** Initialize ball of fluid with the given density gradient.

            \param  ballRadius  Radius of ball of fluid.

            \param  densityGradient 
        */
        DensityGradientBall( float ballRadius , float densityGradient )
            : mRadius( ballRadius )
            , mDensityGradient( densityGradient )
        {
        }
        ...
        virtual void AssignVorton( Vec3 & vorticity , float & density , const Vec3 & position , const Vec3 & vCenter ) const
        {
            ...
            {   // Probe position is inside ball.
                vorticity = RandomSpread( Vec3( 0.001f , 0.001f , 0.001f ) ) ;  // Enough vorticity to emit a vorton, not enough to influence motion
                // density = mDensity * ( mRadius - rho ) ;
                density = mDensityGradient * ( mRadius - rho ) ;
            }
            else
            {
                vorticity = Vec3( 0.0f , 0.0f , 0.0f ) ;
        ...
        float   mRadius             ;   ///< Radius of ball of fluid
        float   mDensityGradient    ;   ///< Density gradient of fluid in ball.
} ;
    ...
    public:
        ...
        */
        DensityBox( const Vec3 & vDimensions , float fDensityDeviation )
            ...
            , mDensity( 1.0f + fDensityDeviation )
        {
        }
        ...
        float   mDensity    ;   ///< Density of fluid in box
} ;
