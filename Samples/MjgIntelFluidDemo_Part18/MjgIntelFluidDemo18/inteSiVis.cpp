...
InteSiVis::InteSiVis( PeGaSys::Render::ApiBase * renderApi )
    ...
    , mFluidScene( & mRenderSystem )

// TODO: Replace render members (mQdCamera, m*Renderer) below with PeGaSys::Render equivalents
#if 0
    , mQdCamera( 320 , 240 )  // Low resolution
#elif 1
    , mQdCamera( 640 , 480 )  // Standard definition
#elif 1
    , mQdCamera( 1280 , 960 ) // 4:3 aspect ratio, high definition
#else
    , mQdCamera( 1280 , 720 ) // 16:9 720p high definition
#endif
...
void InteSiVis::CreateRigidBodyModelsAndEntities()
{
    ...
    mFluidScene.CreateLightsCameraSkybox() ;
...
void InteSiVis::InitialConditions( unsigned ic )
{
    ...
    {   // Switch on initial conditions
        ...
        case 17: // Candle.
        {
            ...
            mQdLights[0].mAmplitudes [ 1 ] = 0.05f ;
            mQdLights[0].mFrequencies[ 1 ] = 1.0f ;
            mQdLights[0].mAmplitudes [ 2 ] = 0.02f ;
            mQdLights[0].mFrequencies[ 2 ] = 2.0f ;
            mQdLights[0].mAmplitudes [ 3 ] = 0.01f ;
...
mVortonPclGrpInfo.mPclOpVortonSim->mVortonSim.SetSpecificHeatCapacity( 1.0f ) ; // Diagnosing: Does this influence vorton motion?
            ...
            mTracerRendering = TRACER_RENDER_NONE ;
...
/* static */ void InteSiVis::GlutDisplayCallback()
{
    ...
    sInstance->mRenderSystem.UpdateTargets() ;
