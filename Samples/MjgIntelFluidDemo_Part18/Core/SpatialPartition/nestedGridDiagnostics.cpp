/** \file NestedGridDiagnostics.cpp

    \brief Diagnostic routines for templated nested uniform grid container, a hierarchical, octree-like spatial partition.
...
/* static */ void NestedGrid<unsigned>::UnitTest( void )
{
    ...
    for( unsigned iLayer = unsigned( ng0.GetDepth() - 1 ) ; iLayer > 0 ; -- iLayer )
    {
        ...
        for( indexOfParentCell[2] = 0 ; indexOfParentCell[2] < layerParent.GetNumPoints( 2 ) ; ++ indexOfParentCell[2] )
        {
            ...
            for( indexOfParentCell[1] = 0 ; indexOfParentCell[1] < layerParent.GetNumPoints( 1 ) ; ++ indexOfParentCell[1] )
            {
                ...
                for( indexOfParentCell[0] = 0 ; indexOfParentCell[0] < layerParent.GetNumPoints( 0 ) ; ++ indexOfParentCell[0] )
                {
                    ...
                    //DebugPrintf( "layer[%u] decimations={%u,%u,%u} iParent={%2u,%2u,%2u} iChild={%2u,%2u,%2u}\n" , iLayer ,
...
/* static */ void NestedGrid<Vec3>::UnitTest( void )
{
#if defined( _DEBUG )
    static unsigned const num = 1024 ;
    static const Vec3 vRange( 2.0f , 3.0f , 5.0f ) ; // range of random positions
    Vec3 vMin( -0.5f * vRange ) ;   // Minimum coordinate of UniformGrid.
    Vec3 vMax(  0.5f * vRange ) ;   // Maximum coordinate of UniformGrid.
    UniformGridGeometry ug( num , vMin , vMax , true ) ;

    NestedGrid<Vec3> ng0( ug ) ;

    extern void UniformGrid_AssignTestValues( UniformGrid<Vec3> & ) ;
    UniformGrid_AssignTestValues( ng0[ 0 ] ) ;
    ng0[ 0 ].GenerateBrickOfBytes( "ngRoot" , 0 ) ;

    for( unsigned iLayer = 1 ; iLayer < ng0.GetDepth() ; ++ iLayer )
    {
        const size_t maxDim = MAX3( ng0[ iLayer ].GetNumPoints( 0 ) , ng0[ iLayer ].GetNumPoints( 1 ) , ng0[ iLayer ].GetNumPoints( 2 ) ) ;
        if( maxDim > 2 )
        {
            ng0.DownSampleInto( iLayer ) ;
            ng0[ iLayer ].GenerateBrickOfBytes( "ng0DownSampled" , iLayer ) ;
        }
    }
    for( unsigned iLayer = 1 ; iLayer < ng0.GetDepth() ; ++ iLayer )
    {
        ng0.UpSampleFrom( iLayer ) ;
        ng0[ iLayer-1 ].GenerateBrickOfBytes( "ng0UpSampledSilly" , iLayer-1 ) ;
    }
    ...
    {
        ng0.UpSampleFrom( iLayer ) ;
        ng0[ iLayer-1 ].GenerateBrickOfBytes( "ng0UpSampled" , iLayer-1 ) ;
    }
#endif
}
