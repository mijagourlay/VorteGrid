...
/*! \brief Initialize object to set render material
    ...
    \note This can only run after the device is initialized.
            ...
            in constructor.
*/
void QdMaterial::Initialize( TextureProc textureProc , int texWidth , int texHeight , float gamma )
{
    ...
    static const unsigned numChannels   = 4 ;
    unsigned char * pImgData = (unsigned char *) malloc( texWidth * texHeight * numChannels ) ;

    switch( textureProc )
    {
        case TP_NOISE_OPAQUE:
            texImgMakeNoise( pImgData , texWidth , texHeight ) ;
            texImgSmooth( pImgData , texWidth , texHeight , numChannels , /* num smoothing passes */ 1 ) ;
            texImgGammaCorrect( pImgData , texWidth , texHeight , gamma ) ;
            break ;

        case TP_NOISE_BALL:
            texImgMakeNoise( pImgData , texWidth , texHeight ) ;
            texImgSmooth( pImgData , texWidth , texHeight , numChannels , /* num smoothing passes */ 1 ) ;
            texImgFadeAlphaRadially( pImgData , texWidth , texHeight , 0.5f , 0.025f ) ;
            texImgGammaCorrect( pImgData , texWidth , texHeight , gamma ) ;
            break ;
    }
