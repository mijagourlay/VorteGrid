...
#if ( ( VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_TREE ) || ( VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_MONOPOLES ) || ( VELOCITY_TECHNIQUE == VELOCITY_TECHNIQUE_DIRECT ) )
    #define USE_PARTICLE_IN_CELL 0
#endif

/** Enable magic buffer to diagnose "jerking".

    Hypothesis: jerking comes from sample points residing inside a region close
    to vortex cores where velocity is very high.  As vortons pass over sample
    points, the velocity temporarily jumps up, then suddenly drops again as those
    vortons rapidly advect away.
    In "real life" this would not happen since a vorton sits precisely at the
    center of itself, where velocity due to that vorton is zero.  Although it's
    perhaps defensible to say that the velocity at the sample point is indeed
    high, it's also more circular, not linear, so the linear interpolation of
    that velocity is incorrect.
*/
#if 1
    #define PLUS_MAGIC_BUFFER
#else
    #define PLUS_MAGIC_BUFFER + 0.1f
#endif
...
// Ideally, sAvoidSingularity should be small enough not to measurably impact
// legitimate values, (i.e. should be smaller than epsilon times the value being
// divided) and should also be larger than a denormalized floating point value
// (which sometimes become zero).
...
{                                                                                                               \
    ...
    const float         radius1             = mSize * 0.5f ;                                                    \
    const float         radius2             = radius1 * radius1 ;                                               \
                ...
                {   // This particle is valid.
                    ...
                    for( size_t jPcl = iPcl ; jPcl < numParticles ; ++ jPcl )
                    {
                        ...
                        {   // This particle is valid.
                            ...
                            const float         den     = pow( dist2 + r2 , 1.5f ) ;
