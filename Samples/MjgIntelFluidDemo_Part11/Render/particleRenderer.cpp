    ...
    {   // VBO extention data has not yet been initialized
	    ...
	    {   // This machine supports Vertex Buffer Objects.
			...
			{	// Failed to find routines that support vertex buffer objects.
                ...
                (void) glDebugOutputSupported , glVendor , glRenderer , glVersion , glExtensions ;
				gVboSupported = false ;
			}
	    }
    }
...
*/
ParticleRenderer::ParticleRenderer( const char * pParticleData , size_t stride , size_t offsetToAngVel , size_t offsetToSize , size_t offsetToDensity , ParticleMaterial & material )
    : mParticleData( pParticleData )
    , mStride( stride )
    , mOffsetToAngVel( offsetToAngVel )
    , mOffsetToSize( offsetToSize )
    , mOffsetToDensity( offsetToDensity )
    , mVertexBuffer( 0 )
...
void ParticleRenderer::FillVertexBufferSlice( const double & timeNow , const Mat4 & viewMatrix , size_t iPclStart , size_t iPclEnd )
{
    ...
    const bool isDensitySigned  = mMaterial.GetNumTexPages() > 1 ;
    const float v0Heavy         = isDensitySigned ? texCoordForPage0of2( 0.01f ) : 0.0f ;
    const float v1Heavy         = isDensitySigned ? texCoordForPage0of2( 0.99f ) : 1.0f ;

#if USE_ORIENTED_SORTED_PARTICLES
    // // Oriented sorted particles do not currently support signed density.
    ...
    {   // For each particle in this slice...
        ...
        Vec4            pclRight    = (   viewRight * cosAngle + viewUp * sinAngle ) * rHalfSize ;
        Vec4            pclUp       = ( - viewRight * sinAngle + viewUp * cosAngle ) * rHalfSize ;
        //const char *    pDensity    = pPos + mOffsetToDensity ;
        //const float &   rDensity    = * ( (float*) pDensity ) ;
    ...
    const float v0Light         = isDensitySigned ? texCoordForPage1of2( 0.01f ) : 0.0f ;
    const float v1Light         = isDensitySigned ? texCoordForPage1of2( 0.99f ) : 1.0f ;
    #if USE_SEPARATE_VBOS
    if( gVboSupported )
    {
        ...
        {   // Assign texture coordinates.
            ...
            {   // For each particle in this slice...
                ...
                // Assign which texture page to use based on sign of density variation.
                ...
                // density sign does not vary and that particles do not change indices (i.e.
                // that they do not die) or that particle density sign is uniform across all particles.
                const float v0 = rDensity >= 0.0f ? v0Heavy : v0Light ;
                const float v1 = rDensity >= 0.0f ? v1Heavy : v1Light ;
        ...
        {   // For each particle in this slice...
            ...
            {   // These are luminous particles.  Vary brightness (not opacity).
                colorMod = MIN2( 255 , fabsf( rDensity ) * mMaterial.GetDensityVisibility() ) ;
            }
            else
            {   // These are translucent particles.  Vary opacity (not color).
                alphaMod = MIN2( 255 , fabsf( rDensity ) * mMaterial.GetDensityVisibility() ) ;
            }
...
void ParticleRenderer::Render( double timeNow , float timeChange , size_t numParticles )
{
    ...
    (void) timeChange ; // Avoid "unreferenced formal parameter" warning.
